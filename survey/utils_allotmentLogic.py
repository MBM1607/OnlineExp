...
def openNodesT2(_treatement,politicalLeaning):
    ...
    try:
        openNode = Treatement.objects.filter(status="awaitingParticipant", participantTypeRequired=politicalLeaning)[0]
        return openNode
    except Exception as e:
        return None
...
def openNodes(_treatement):
    ...
    try:
        openNode = Treatement.objects.filter(status="awaitingParticipant")[0]
        return openNode
    except Exception as e:
        return None
